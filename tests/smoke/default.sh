...
printf '%s\n' "$version_output" | grep -F "BitTorrent client version 0.16.19." >/dev/null \
  || fail 'rTorrent 0.16.19 version'
docker exec "$container" sh -c "strings /usr/local/lib/libtorrent.so.49 | grep -F 'libTorrent 0.16.19' >/dev/null" \
  || fail 'libtorrent 0.16.19 version'
internal_versions=$(docker exec --user 991:991 --workdir /rutorrent/app/php "$container" php85 -r '
  ...
  if (!$settings->linkExist || $settings->version !== "0.16.19" || $settings->libVersion !== "0.16.19") {
...
[ "$internal_versions" = '0.16.19/0.16.19' ] \
...
grep -F '0.16.19' "$rpc_response" >/dev/null || fail 'RPC2 library version response'
