...
printf '%s\n' "$version_output" | grep -F "BitTorrent client version 0.16.18." >/dev/null \
  || fail 'rTorrent 0.16.18 version'
docker exec "$container" sh -c "strings /usr/local/lib/libtorrent.so.48 | grep -F 'libTorrent 0.16.18' >/dev/null" \
  || fail 'libtorrent 0.16.18 version'
internal_versions=$(docker exec --user 991:991 --workdir /rutorrent/app/php "$container" php85 -r '
  ...
  if (!$settings->linkExist || $settings->version !== "0.16.18" || $settings->libVersion !== "0.16.18") {
...
[ "$internal_versions" = '0.16.18/0.16.18' ] \
...
grep -F '0.16.18' "$rpc_response" >/dev/null || fail 'RPC2 library version response'
