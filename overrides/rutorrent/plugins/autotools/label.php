...
if( $is_ok && strpos( $label, "{DIR}" ) !== false )
{
	$req = new rXMLRPCRequest( array (
		...
		new rXMLRPCCommand( getCmd("d.get_custom3"),   $hash ),
...
if( $is_ok && strpos( $label, "{TRACKER}" ) !== false )
{
	$req = new rXMLRPCRequest( array(
		new rXMLRPCCommand( getCmd("t.multicall"),
			// Version-aware multicall keeps compatibility with older rtorrent while using t.multicall2 when available.
...
{
	Debug( "label           : \"".$label."\"" );
	if( ($label != "") && rtExec( getCmd("d.set_custom1"), array( $hash, UTF::raw_url_encode( $label ) ), $autodebug_enabled ))
