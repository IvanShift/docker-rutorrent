...
class rRatio
{
	...
	public function getTimes()
	{
		...
		private function viewListCommand()
		{
	...
	public function correct()
	{
				...
				foreach($req->strings as $no=>$hash)
				{
			        for($i=0; $i<MAX_RATIO; $i++)
			        {
						if($req->i8s[$no*MAX_RATIO*2+$i*2]==1)
							$req1->addCommand(new rXMLRPCCommand(getCmd("view.set_visible"),array($hash,"rat_".$i)));
					}						
				}
				return(($req1->getCommandsCount()==0) || ($req1->success()));
			}
		...
		{
			$req1 = new rXMLRPCRequest(new rXMLRPCCommand($this->viewListCommand()));
			if($req1->run() && !$req1->fault)
			{
				...
				for($i=0; $i<MAX_RATIO; $i++)
				{
					...
					if(!in_array("rat_".$i,$req1->val))
						foreach($this->insertPersistentViewCommands("rat_".$i) as $cmd)
							$req->addCommand($cmd);
					if($this->isCorrect($i))
					{
