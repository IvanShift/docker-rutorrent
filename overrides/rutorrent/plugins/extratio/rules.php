...
class rRatioRulesList
{
	...
	public function checkLabels( $hashes )
	{
		...
		{
			$req->addCommand( new rXMLRPCCommand( getCmd("d.get_custom1"), $hash ) ); 
			$req->addCommand( new rXMLRPCCommand( getCmd("d.get_state"), $hash ) );
			...
			$req->addCommand( new rXMLRPCCommand( getCmd("d.get_throttle_name"), $hash ) );
		}
		if($req->getCommandsCount() && $req->success())
		{
			...
			foreach( $hashes as $ndx=>$hash )
			{
				...
				$trackers = '';
					...
					// Version-aware multicall to keep tracker list compatible with newer rtorrent APIs.
					new rXMLRPCCommand(getCmd("t.multicall"), 
				...
				if($rule)
				{
					if(!empty($rule->channel) && ($rule->channel!=$throttle))
					{
						...
						$out->addCommand( new rXMLRPCCommand(getCmd('d.set_throttle_name'), array($hash,$rule->channel)) );
						if($state)
					...
					if(!empty($rule->ratio) && ($rule->ratio!=$ratio))
					{
						if(!is_null($ratio))
						{
							$out->addCommand( new rXMLRPCCommand(getCmd('view.set_not_visible'), array($hash, $ratio) ) );
							$out->addCommand( new rXMLRPCCommand(getCmd('d.views.remove'), array($hash, $ratio) ) );
						}
						$out->addCommand( new rXMLRPCCommand('d.views.push_back_unique', array($hash, $rule->ratio) ) );
						$out->addCommand( new rXMLRPCCommand(getCmd('view.set_visible'), array($hash, $rule->ratio) ) );
					}
				}
			}
