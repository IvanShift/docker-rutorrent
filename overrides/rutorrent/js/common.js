			...
			if (arr[i] == null)
				...
				switch (table.getIdByCol(i)) {
					...
					case 'version':
						var nameCol = -1;
						for (var j in arr) {
							if (table.getIdByCol(j) == 'name') {
								nameCol = j;
								break;
							}
						}
						if ((nameCol >= 0) && thePlugins.get(arr[nameCol]) && thePlugins.get(arr[nameCol]).versionString)
							arr[i] = thePlugins.get(arr[nameCol]).versionString;
						else {
							var n = Number(arr[i]);
							arr[i] = isNaN(n) ? arr[i] : n.toFixed(2);
						}
						break;
				}
		}
